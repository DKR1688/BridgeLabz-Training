...
-- Altering table to add and drop column
ALTER TABLE specializations ADD COLUMN experience_years INT;
ALTER TABLE specializations DROP COLUMN experience_years;

-- One UPDATE and one DELETE query with proper WHERE clauses, and verify results with SELECT before and after.
...
-- A new MySQL user clinic_app_user with SELECT, INSERT, UPDATE, DELETE privileges only on health_clinic_db
