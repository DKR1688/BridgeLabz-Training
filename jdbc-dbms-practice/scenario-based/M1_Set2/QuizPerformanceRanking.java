    ...
    public void topQuiz(String quizName) {
        ...
        switch (quizName) {
            case "Q1":
                ...
                for (Student s : records) if (s.q1 == max) System.out.println(s.name + " " + s.q1);
                break;
            case "Q2":
                max = records.stream().mapToInt(s -> s.q2).max().orElse(-1);
                for (Student s : records) if (s.q2 == max) System.out.println(s.name + " " + s.q2);
                break;
            case "Q3":
                max = records.stream().mapToInt(s -> s.q3).max().orElse(-1);
                for (Student s : records) if (s.q3 == max) System.out.println(s.name + " " + s.q3);
                break;
            default:
