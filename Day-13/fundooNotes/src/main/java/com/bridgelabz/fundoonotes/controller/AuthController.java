...
public class AuthController {
    ...
    private static final Map<String, String> RECOVERY_RESPONSE = Map.of("message",
    ...
    @PostMapping({ "/register", "/userSignUp" })
    public ResponseEntity<AuthResponse> register(@Valid @RequestBody RegisterRequest request) {
        String token = userService.register(
                request.email(),
                request.password(),
                request.resolvedName(),
                request.resolvedFirstName(),
                request.resolvedLastName());
